...
- **ZENN THREE-BEAT HOOK (narrated/explainer formats — the highest-retention structure measured across top explainer creators):** compress the opening into three quick beats instead of one long sentence. Beat 1, PRESENT MOMENT: drop the viewer straight into the situation in motion ("The temperature in this server room is dropping one degree every minute" — present tense, in-progress, never setup). Beat 2, RUG PULL: the detail that flips what they just pictured ("because the owner just deleted the power bill payment"). Beat 3, SEALED OPEN LOOP: name the stakes/question the rest of the script will resolve, stated as a closed commitment ("and what happens when it hits zero decides whether this company survives"). The whole three beats should land inside the first 4-6 seconds, one short sentence each. This gives the viewer two separate micro-surprises before you've even started the explanation.
- **SNAP-BACK CLOSE (narrated formats):** the final sentence must snap the topic directly onto the viewer personally — one concrete thing this fact means for THEM, in their life, right now ("…which means the bug you filed this morning is already being sold back to you as a fix"). A close that lands on the viewer outranks any generic summary or moral. If LOOP_MODE=true, fold the snap-back into the loop tail so the replay still lands personally.
